...
/* osfweb version: 16.0.15201.10000 */
/* office-js-api version: 20220404.2 */
/*
...
Microsoft.Office.Common.ResponseSender = function Microsoft_Office_Common_ResponseSender(requesterWindow, requesterUrl, actionName, conversationId, correlationId, responseType, serializerVersion) {
    ...
    this._send = function (result) {
        try {
            var postMessageTarget = me._requesterUrl;
            if (me._actionName === "dialogMessageReceived" || me._actionName === "dialogParentMessageReceived") {
                if (!result.targetOrigin) {
                    ;
                }
                else {
                    postMessageTarget = result.targetOrigin;
                }
            }
            ...
            me._requesterWindow.postMessage(envelopedResult, postMessageTarget);
            ;
        }
        catch (ex) {
...
OSF.InitializationHelper.prototype.getAppContext = function OSF_InitializationHelper$getAppContext(wnd, gotAppContext) {
    ...
    var getInvocationCallbackWebApp = function OSF_InitializationHelper_getAppContextAsync$getInvocationCallbackWebApp(errorCode, appContext) {
        ...
        if (errorCode === 0 && appContext._id != undefined && appContext._appName != undefined && appContext._appVersion != undefined && appContext._appUILocale != undefined && appContext._dataLocale != undefined &&
            ...
            returnedContext._marketplaceType = appContext._marketplaceType;
            if (OSF.AppTelemetry) {
...
OSF.InitializationHelper.prototype.addOrRemoveEventListenersForWindow = function OSF_InitializationHelper$addOrRemoveEventListenersForWindow(isAdd) {
    ...
    var onWindowKeydown = function (e) {
        ...
        if (e.keyCode == 117 && (e.ctrlKey || e.metaKey)) {
            ...
            var isPowerPointModernSlideShow = me._appContext._isPowerPointModernSlideShowEnabled &&
...
(function (OSFAppTelemetry) {
    ...
    function initialize(context) {
        ...
        appInfo.hostJSVersion = "16.0.15201.10000";
    ...
    function sendPerformanceTelemetry() {
        if (typeof OTel !== "undefined" && OSF.AppTelemetry.enableTelemetry && typeof OSFPerformance !== "undefined" && typeof (performance) != "undefined" && performance.getEntriesByType) {
            ...
            resources.forEach(function (resource) {
                ...
                if (OSF.OUtil.stringEndsWith(resourceName, OSFPerformance.hostSpecificFileName)) {
                    hostPerfResource_1 = resource;
                }
                else if (OSF.OUtil.stringEndsWith(resourceName, OSF.ConstantNames.OfficeDebugJS) ||
                    OSF.OUtil.stringEndsWith(resourceName, OSF.ConstantNames.OfficeJS)) {
...
OSF.DDA.UI.ParentUI = function OSF_DDA_ParentUI() {
    ...
    if (Microsoft.Office.WebExtension.FeatureGates && Microsoft.Office.WebExtension.FeatureGates["OfficeJsDialogRefactor"]) {
        ...
        var defineDialogApi = function (apiName, isModalApi) {
            if (!target_1[apiName]) {
                OSF.OUtil.defineEnumerableProperty(target_1, apiName, {
                    value: function () {
                        var openDialog = OSF._OfficeAppFactory.getHostFacade()[OSF.DDA.DispIdHost.Methods.OpenDialog];
                        openDialog(arguments, eventDispatch, target_1, isModalApi);
                    }
                });
            }
        };
        ...
        var displayDialogApiName = OSF.DDA.AsyncMethodNames.DisplayDialogAsync.displayName;
        var target_2 = this;
        if (!target_2[displayDialogApiName]) {
            OSF.OUtil.defineEnumerableProperty(target_2, displayDialogApiName, {
                value: function () {
                    var openDialog = OSF._OfficeAppFactory.getHostFacade()[OSF.DDA.DispIdHost.Methods.OpenDialog];
                    openDialog(arguments, eventDispatch, target_2, false);
                }
            });
        }
    }
        ...
        (function (Dialog) {
            ...
            function sendMessage(params) {
                if (windowInstance != null) {
                    ...
                    windowInstance.postMessage(message, targetOrigin);
                }
            }
                ...
                if (event.source == windowInstance) {
                    try {
                        ...
                        if (!taskpaneUrl) {
                            ...
                            if (!taskpaneUrl) {
                                taskpaneUrl = window.location.protocol + "//"
                                    + window.location.hostname
                                    + (window.location.port ? ':' + window.location.port : '');
                            }
                        }
            ...
            function checkAppDomain(dialogInfo) {
                ...
                if (!fInDomain) {
                    ...
                    return taskpaneUrl && Microsoft.Office.Common.XdmCommunicationManager.isTargetSubdomainOfSourceLocation(taskpaneUrl, url);
                }
                return fInDomain;
            }
...
var Features = {
  ...
  replyCallback: production,
  sessionData: production
};
...
// CONCATENATED MODULE: ./src/utils/isOfficeVersion.ts


var isOfficeVersion_isOfficeVersionOrGreater = function isOfficeVersionOrGreater(officeVersion) {
  if (!isNullOrUndefined(officeVersion)) {
    var hostVersionParts = getInitialDataProp("hostVersion").split(".");
    var officeVersionParts = officeVersion.split(".");
    var minPartsLength = 4;
    var comparisons = 0;

    if (hostVersionParts.length >= minPartsLength && officeVersionParts.length >= minPartsLength) {
      for (var i = 0; i < minPartsLength; i++) {
        var hostVersionValue = parseInt(hostVersionParts[i]);
        var officeVersionValue = parseInt(officeVersionParts[i]);

        if (isNaN(hostVersionValue) || isNaN(officeVersionValue) || hostVersionValue < officeVersionValue) {
          return false;
        }

        if (hostVersionValue > officeVersionValue) {
          return true;
        }
      ...
      return comparisons == minPartsLength;
    }
  }

  return false;
};
// CONCATENATED MODULE: ./src/utils/isHostPlatform.ts
var isHostPlatform_OSF = __webpack_require__(0);

var isHostPlatform = function isHostPlatform(platform) {
  return isHostPlatform_OSF._OfficeAppFactory.getHostInfo()["hostPlatform"] == platform;
};
...
function validateInsightMessageParameters(data) {
  ...
  if (!isHostPlatform("win32") || !isOfficeVersion_isOfficeVersionOrGreater("16.0.14620.10000")) {
    if (!isNullOrUndefined(data.persistent)) {
      throw createArgumentError(NotificationsPersistentParameterName);
    }
  }
...
};function exposeOfficeRuntimeThroughOfficeNamespace(OfficeRuntime, Office) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
    OfficeRuntime = typeof OfficeRuntime !== "undefined" ? OfficeRuntime : (_a = window) === null || _a === void 0 ? void 0 : _a.OfficeRuntime;
...
exposeOfficeRuntimeThroughOfficeNamespace(OfficeRuntime, Office);
if (typeof OSFPerformance !== "undefined") {
